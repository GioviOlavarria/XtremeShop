        ...
        if (items.isEmpty()) {
            ...
            Column(
                ...
                Column(
                    ...
                    Button(
                        onClick = { nav.navigate(Rutas.CHECKOUT) },
...
private fun CartRow(
    ...
    ListItem(
        ...
        supportingContent = {
            Text(
                "x${item.quantity} • $${"%.0f".format(item.unitPrice)} = " +
                        "$${"%.0f".format(item.unitPrice * item.quantity)}"
            )
        },
        trailingContent = {
            Row(verticalAlignment = Alignment.CenterVertically) {
                IconButton(onClick = onDec) { Icon(Icons.Filled.Remove, "Disminuir") }
                Text("${item.quantity}", modifier = Modifier.padding(horizontal = 8.dp))
                IconButton(onClick = onInc) { Icon(Icons.Filled.Add, "Aumentar") }
